...
### Define the StateEndorse, and EnvironmentEndorse classes for the endorse model
...
class EnvironmentEndorse:
    def __init__(self, jobs, candidates):
        self.jobs = jobs
        self.candidates = candidates
        self.current_state = StateEndorse(self.jobs, self.candidates)
    ...
    def step(self, action):
        ...
        if len(self.available_actions) != 0:
            ...
            # Fairness penalty
            if self.candidates[candidate].gender == 2:
                women_count = sum(1 for c in self.candidates if c.allocation_status == 1 and c.gender == 1)
                men_count = sum(1 for c in self.candidates if c.allocation_status == 1 and c.gender == 2)

                fairness_penalty = max(0, men_count - women_count) * 0.1 
                reward *=(1 + fairness_penalty)
